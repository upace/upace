...
(function($) {
	...
	var Mapplic = function() {
		...
		self.init = function(el, params) {
			...
			// Check this is a json object or link
			if(typeof params.source == 'object')
			{
				processData(params.source);
		...
		function DragPointer() {
			...
			this.init = function() {
				...
				//alert('hi');
				// Construct
				//$('.mapplic-layer a').addClass("ui-widget-content");
				
				//document.ondragstart = function() { return false; } // IE drag fix

					// Drag & drop
					$('.mapplic-layer a').on('mousedown', function(event) {
						//console.log($('.mapplic-coordinates-x').html() + ' - ' + event.pageY);
						var t_a = $(this);

						/*t_a.on('mousemove', function(event) {
							var t_x = $('.mapplic-coordinates-x').html();
							var t_y = $('.mapplic-coordinates-y').html();
							var t_left = parseFloat(t_x) * 100;
							var t_top = parseFloat(t_y) * 100;
						...
						$(document).on('mouseup', function(event) {
							...
							update_equipment(equipId,t_x,t_y);
						});
						/*map.stop();
		...
		var processData = function(data) {
					...
					$.each(value.locations, function(index, value) {
						...
						if (value.pin != 'hidden') {
							if (self.o.locations) {
								...
								var pin = $('<a></a>').attr('href', target).addClass('mapplic-pin').addClass(value.type).css({'top': top + '%', 'left': left + '%'}).appendTo(layer);
			...
			// Draggable
			if(self.o.draggable) self.draggable = new DragPointer().init();
