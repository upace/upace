		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
                	...
                	success: function(qOccRes){
                	 for(j in qOccRes)
                	 {
						...
						/******************* Check Gym Start time and End Time *********************/
							...
							//while(start_time != end_time);
						/******************* Check Gym Start time and End Time *********************/
                	 	getDataChartGym(gymId,j,qOccRes.length,rmId,gym_start_time,gym_end_time);
                	 }
                	}
                })
				...
				function getDataChartGym(gymId,j,qLen,rmId)
                {
					...
					//console.log('start_time - ' + gym_start_time + ' / ' + gym_end_time);
					/*gym_start = gym_start.split(":");
					gym_start_ap = gym_start['1'].split(" ");
					if(gym_start_ap == 'AM')
					{
						gym_start_time = parseInt(gym_start['0']);
					}
					else
					{
						gym_start_time = parseInt(gym_start['0']) + 12;
					}

					gym_end = gym_end.split();*/
							...
							for(i in results)
							{
				              ...
				              if(room)
				              {
				                  ...
				                  if(moment(date).format('YYYY/MM/DD') == checkDate )
								  {
									  ...
									  var row;
									  console.log(moment(date).format('hh A'));
									  /*if(!old_date || (moment(date).format('hh A')!=moment(old_date).format('hh A')))
										  {
											   row = '<li style="clear:both;list-style: outside none none;"></li>';
										  }
										  else{
											row = '';
										  }*/
