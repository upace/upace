      ...
      success: function(results){
         for(i in results){
            ...
            if(room)
            {
				...
				row +=          '<input id="st'+c+'" class="onoffswitch-checkbox" type="checkbox" onclick="change_roomAllowOccupancy(\''+ room.id +'\',\''+ gymId +'\')"';
         ...
         for(i in results){
            ...
            { 
                if(room.get('allowOccupancy')==true)
                {
		           ...
		           //row +=      '<a class="btn btn-primary" href="/calendar"><i class="fa fa-eye"></i>View Classes</a>';
		           //row +=      '<a class="btn btn-primary" href="/viewEquipments"><i class="fa fa-eye"></i>View Equipment History</a>'
		           row +=      '<a class="btn btn-primary" href="viewGymOccupancy?gid='+gymId+'&rid='+room.id+'"><i class="fa fa-eye"></i>View</a>'
    ...
    ug.find({
        ...
        {   
		     ...
		     for(i in res){
		         ...
		         var classes = reserve.get("class");
		         var user = reserve.get("user");
		         getReserveUserDetails(user, classes, c++, len, i, reserve);
		     }
		    // $('.projects-table').dataTable();
...
function getReserveUserDetails(user, classes, c, len, i, reserve)
{
        ...
        success:function(results){
            ...
            if(classes)
            {
                ...
                row += '<td>'+moment(classes.get('date'),'DD.MM.YYYY').format('MM/DD/YYYY')+'</td>';
				 row += '<td>'+moment(classes.get('date'),'DD.MM.YYYY').format('dddd')+'</td>';
        ...
        success:function(results)
        {
            ...
            for(i in results)
            {
		       ...
		       if(classes)
		       {
		           ...
		           row += '"description" : "'+classes.get('name')+'",';
		           row += '"className" : ["event", "bg-color-greenLight"]';
		           row += '}';
		           //console.log(i);
			     if(parseInt(length-1)==i)
