...
function getAllUserList(user,len,c)
{
	var ug = Parse.Object.extend('university_gym');
	var ugQuery = new Parse.Query(ug);
	ugQuery.equalTo('objectId',user.get('universityGymId'));
	ugQuery.first({
		success: function(ugRes){
         ...
         for(i in results){
            ...
            if(room)
            {
              ...
              {  
                ...
                row += '<td>';
                //row +=      '<a class="btn btn-primary" href="/calendar"><i class="fa fa-eye"></i>View Classes</a>';
                row +=      '<a class="btn btn-primary" href="viewGymOccupancy?gid='+gym.id+'&rid='+room.id+'"><i class="fa fa-eye"></i>View</a>'
              ...
              //  row +=      '<a class="btn btn-primary" href="javascript:void(0);" onclick="viewGymOccupancy(\''+room.id+'\')"><i class="fa fa-crosshairs"></i>View</a>';
                row += '</td>';
                row += '</tr>';
                $('table.rooms tbody').append(row);
              }  
						                         ...
						                         success:function(){
						                             ...
						                             if(tot_row == 0)
						                             {
														...
														if(!login_count)
														{
															...
															up_count.save(null,{
																success:function(){
																	window.location = '/admin/addEquipments';
																}
															})
														}
															else
...
{
   // var baseurl= 'http://server3-upace.vm-host.net/admin/';
	var gymId = currentUser.get('universityId');
    ...
    q.find({
		...
		//console.log(new Date(y, m, d,start_tym[0],start_tym[1] ));	            
				   ...
				   row += '"url" : "viewReservation?rid='+class_slot.id+'&sid='+classes.id+'"';
			   ...
			   /* $('#add-event').click(function () {
			    ...
			    $('#calendar').fullCalendar({
			        ...
			        drop: function (date, allDay) { // this function is called when something is dropped
						...
						$('#calendar').fullCalendar('eventLimit', copiedEventObject, true);
    ...
    q.first({
         success: function(results){
			get_rooms_list(results.get('gymId'));
			 ...
			 calenderClass(results.get('universityId'));
			 //console.log("Room Id - " + results.get('roomId'));
...
function add_classReservation(data)
{
    ...
    //comsole.log('slotId ' + slotId + '|classId' + classId +'|user'+user);
    //console.log('Equipment Id ' + equipment);
...
function getReserveUserDetails(user, gym, classes, c, len, i, reserve)
{
	if(user && gym && classes)
	{
	var pay = Parse.Object.extend('user_payment');
    ...
    q.find({
		...
		//console.log(new Date(y, m, d,start_tym[0],start_tym[1] ));	            
				  ...
				  // row += '"end" : "'+(moment(classes.get('date'),'DD.MM.YYYY').format('YYYY-MM-DD')+'T'+end_tym[0]+':'+end_tym[1])+':00",';
				   ...
				   row += '"link" : "viewReservation?rid='+class_slot.id+'&sid='+classes.id+'",';
					...
					cs.find({
						success:function(all_slots)
						{
							if(all_slots)
							{
								...
								//comsole.log('total_slot - ' + total_slot);
								for(i in all_slots)
								{
									if(total_slot==1)
									{
