                    ...
                    query.first({
                    			success: function (user) {
									...
									document.getElementById('Submit').style.pointerEvents = 'auto';
                    					...
                    					var University = Parse.Object.extend("university");
									var b1 = new University({"name":university,"email":email,"firstname":firstname,"lastname":lastname,"totalGym":parseInt(totalGym),"isActive":1});
									b1.save(null, {
									    success:function(b1){
										 console.log(b1.id);
										  var sendMail = 1;
...
function getStaffRating(){
    ...
    q.find({
      success: function(results){
         for(i in results){
            	var gymStaff = results[i];
				var univ = gymStaff.get('university');
				var gym = gymStaff.get('gym');
            	console.log(results.length);
            	 getRating(gymStaff,i,results.length,gym,univ);
...
function getRating(gymStaff,c,len,gym,univ)
{
